...
class CommunityStoryViewerCubit extends BaseCubit<CommunityStoryViewerState> {
  ...
  Future<void> _handleCommunityEvent(AppCommunityEvent event) async {
    ...
    final currentOwnerId = state.stories.isEmpty ? '' : state.stories.first.userId;
    ...
    if (event.table == 'community_stories' &&
        (affectedStoryId == state.storyId || affectedUserId == currentOwnerId)) {
      _patchStories(event);
      return;
    }
  ...
  void _patchStories(AppCommunityEvent event) {
    final nextStories = switch (event.action) {
      ...
      AppCommunityRealtimeAction.deleted => state.stories
  ...
  double _doubleValue(Map<String, dynamic> record, String key, double fallback) {
    final value = record[key];
    if (value is num) {
      return value.toDouble().clamp(0.0, 1.0);
    }

    return (double.tryParse(value?.toString() ?? '') ?? fallback).clamp(0.0, 1.0);
  }
