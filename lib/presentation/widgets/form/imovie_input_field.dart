...
class IMovieInputField extends StatelessWidget {
  const IMovieInputField({
    super.key,
    required this.state,
    this.label = 'Label',
  ...
  Widget build(BuildContext context) {
    final palette = _IMovieFieldPalette.fromState(state);
...
class _IMovieFieldPalette {
  const _IMovieFieldPalette({
    required this.borderColor,
    required this.supportingColor,
  });

  factory _IMovieFieldPalette.fromState(IMovieFieldState state) {
    return switch (state) {
      IMovieFieldState.defaultState => const _IMovieFieldPalette(
        borderColor: AppColors.surfaceStroke,
        supportingColor: AppColors.textSecondary,
      ),
      IMovieFieldState.typing => const _IMovieFieldPalette(
        borderColor: AppColors.yellow500,
        supportingColor: AppColors.textSecondary,
      ),
      IMovieFieldState.error => const _IMovieFieldPalette(
        borderColor: AppColors.danger,
        supportingColor: AppColors.danger,
      ),
      IMovieFieldState.filled => const _IMovieFieldPalette(
